...
void printError(const char* currFileName, ERROR_CODE errorCode, int lineIndex){
    ...
    if (errorCode == INITIALIZING) {
        fileName = calloc(strlen(currFileName) + strlen(".as") + 1, sizeof(char));
        strcpy(fileName, currFileName);
        strcat(fileName, ".as");
        return;
    }else if(errorCode == FINISHIZING){
        if (fileName) {
            free(fileName);
        }
        return;
    ...
    switch (errorCode) {
        ...
        case NUMBER_OUT_OF_BOUNDS_10BITS:
            printf("\nError at file: %s, line %d, all data values should be numbers in a range from %d to %d\n", fileName, lineIndex, MIN_10BIT_NUMBER, MAX_10BIT_NUMBER);
            break;
